...
send_pkt_out(char *pkt, unsigned int pkt_size, interface_t *interface) {
    ...
    if (!nbr_node)
        return -1;
    unsigned int dest_udp_port_num = nbr_node->udp_port_number;
    int sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (sock<0) {
        printf("Error: Sending socket creation failed. errno = %d",errno);
        return -1;
    }    
    interface_t *other_intf = (interface == &interface->link->intf1) ? &interface->link->intf2 : &interface->link->intf1;
    ...
    rc = _send_pkt_out(sock, pkt_with_aux_data, pkt_size+IF_NAME_SIZE, dest_udp_port_num);
    close(sock);
    return rc;
}
int
pkt_receive(node_t *node, interface_t *interface, char *pkt, unsigned int pkt_size) {
    pkt = pkt_buffer_right_shift(pkt, pkt_size, MAX_PACKET_BUFFER_SIZE-IF_NAME_SIZE);
...
_pkt_receive(node_t *receiving_node, char *pkt_with_aux_data, unsigned int pkt_size) {
    ...
    if (!recv_intf) {
        printf("Packet received on unknown interface %s on node %s",recv_intf->if_name, receiving_node->node_name);
        return;
    }
    ...
    while(1) {
        ...
        ITERATE_GLTHREAD_BEGIN(&topo->node_list, curr) {
            ...
            if (FD_ISSET(node->udp_socket_fd, &active_sock_fd_set)) {
                ...
                bytes_recvd = recvfrom(node->udp_socket_fd, (char*)recv_buffer, MAX_PACKET_BUFFER_SIZE, 0, (struct sockaddr*)&sender_addr, &addr_len);
